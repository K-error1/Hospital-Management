  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Patients" value={patients.length} icon="🏥" change="+3 this week" changeType="positive" href="/Receptionist/patients" />
        <StatCard title="Active Doctors" value={doctors.filter(d => d.status !== 'On Leave').length} icon="👨‍⚕️" change={`${doctors.length} total`} changeType="neutral" href="/Receptionist/doctors" />
        <StatCard title="Nurses on Duty" value={nurses.filter(n => n.status === 'On Duty').length} icon="👩‍⚕️" change={`${nurses.length} total`} changeType="neutral" href="/Receptionist/nurses" />
        <StatCard title="Bed Occupancy" value={`${Math.round((occupiedBeds / totalBeds) * 100)}%`} icon="🛏️" change={`${occupiedBeds}/${totalBeds} beds`} changeType={occupiedBeds / totalBeds > 0.8 ? 'negative' : 'positive'} href="/Receptionist/departments" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Today's Appointments" value={todayAppointments.length} icon="📅" change="2 completed" changeType="positive" href="/Receptionist/appointments" />
        <StatCard title="Critical Patients" value={criticalPatients.length} icon="🚨" change="Needs attention" changeType="negative" href="/Receptionist/patients" />
        <StatCard title="Total Revenue" value={`Ksh ${(totalRevenue / 1000).toFixed(0)}K`} icon="💰" change="+12% this month" changeType="positive" href="/Receptionist/billing" />
        <StatCard title="Departments" value={departments.length} icon="🏢" change="All operational" changeType="positive" href="/Receptionist/departments" />
      </div>
              ...
              <div key={apt.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-fuchsia-100 rounded-full flex items-center justify-center text-fuchsia-600 font-bold text-sm">
