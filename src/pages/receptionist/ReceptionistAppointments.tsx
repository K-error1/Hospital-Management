...
export default function ReceptionistAppointments() {
  ...
  const columns = [
    ...
    { key: 'status', label: 'Status', render: (a: Appointment) => <StatusBadge status={a.status} /> },
    {
      ...
      render: (a: Appointment) => (
        <button onClick={() => openEdit(a)} className="px-2 py-1 bg-fuchsia-50 text-fuchsia-600 rounded text-xs font-medium hover:bg-fuchsia-100 transition-colors">
          ✏️ Edit
        </button>
      ),
    },
        ...
        {editAppt && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
              ...
              <button onClick={handleSaveAppt} disabled={isSaving} className="px-4 py-2 bg-fuchsia-600 text-white rounded-lg text-sm hover:bg-fuchsia-700 disabled:opacity-50">
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => !isSubmitting && setIsAddModalOpen(false)} title="Schedule Appointment" size="md">
        <form onSubmit={handleAddAppointment} className="space-y-4">
          ...
          <div className="pt-2 flex justify-end gap-3">
            ...
            <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-fuchsia-600 text-white rounded-lg hover:bg-fuchsia-700 font-medium text-sm disabled:opacity-50">
