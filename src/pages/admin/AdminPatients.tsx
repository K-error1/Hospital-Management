...
import { Patient, Doctor, Nurse } from '../../types';
...
export default function AdminPatients() {
  ...
  const loadData = async () => {
    try {
      const [pData, dData, nData] = await Promise.all([api.fetchPatients(), api.fetchDoctors(), api.fetchNurses()]);
      setPatients(pData);
      setDoctors(dData);
      setNurses(nData);
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this patient? They will not be able to log in anymore.')) return;
    try {
  ...
  const handleAddPatient = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newPatient = {
      ...
      assignedNurse: formData.get('assignedNurse'),
  ...
  const getNurseName = (id: string) => nurses.find(n => n.id === id)?.name || 'N/A';
  ...
  const columns = [
    ...
    { key: 'assignedNurse', label: 'Nurse', render: (p: Patient) => getNurseName(p.assignedNurse) },
    ...
    { key: 'status', label: 'Status', render: (p: Patient) => <StatusBadge status={p.status} /> },
    {
      ...
      render: (p: Patient) => (
        <div className="flex gap-3">
          <button onClick={() => openDetails(p)} className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
            View Details
          </button>
          ...
          <div className="space-y-4">
            ...
            {isEditing ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className={labelCls}>Assign Nurse</label>
                  <select value={editData.assignedNurse || ''} onChange={e => setEditData(d => ({ ...d, assignedNurse: e.target.value }))} className={inputCls}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg"><p className="text-xs text-gray-500">Nurse</p><p className="font-medium">{getNurseName(selectedPatient.assignedNurse)}</p></div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
              {isEditing ? (
                ...
                <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700">✏️ Edit Patient</button>
              )}
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleAddPatient} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required name="name" type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="e.g. John Doe" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Age</label>
                <input required name="age" type="number" min="0" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Gender</label>
                <select required name="gender" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input required name="bloodGroup" type="text" placeholder="e.g. O+" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
          ...
          <div>
            ...
            <input required name="address" type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Assign Doctor</label>
              <select required name="assignedDoctor" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
              ...
              <select required name="assignedNurse" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
              ...
              <select required name="status" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="room" type="text" placeholder="e.g. 101-A" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Initial Diagnosis (Optional)</label>
              <input name="diagnosis" type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
